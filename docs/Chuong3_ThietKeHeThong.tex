...
\begin{itemize}
    \item \textbf{Tầng trình bày (Frontend)}
    Phát triển bằng \textbf{Vue.js 3 + Element Plus}.
    Đây là nơi tương tác trực tiếp với: khách hàng, nhân viên phục vụ, nhân viên bếp, quản lý chi nhánh và admin hệ thống.
    Frontend giao tiếp với Backend qua API (HTTP/HTTPS) và WebSocket khi cần đồng bộ realtime.

    \item \textbf{Tầng ứng dụng (Backend)}
    Xây dựng bằng \textbf{Node.js + Express}.
    Chịu trách nhiệm xử lý toàn bộ logic nghiệp vụ được mô tả trong UC01--UC17, gồm:
    \begin{itemize}
        \item quản lý đặt bàn, kiểm tra bàn trống theo \textbf{buffer giữ chỗ 2 giờ}, tự động phân bàn và ghép bàn liền kề;
        ...
        \item tổng hợp hóa đơn, xử lý thanh toán (tiền mặt, chuyển khoản, Momo, VietQR);
        \item quản lý menu, chi nhánh, nhân sự và tài khoản;
        \item thống kê doanh thu và đánh giá khách hàng;
        \item phân quyền người dùng theo vai trò (\texttt{role}) và phạm vi chi nhánh.
    \end{itemize}

    \item \textbf{Tầng dữ liệu (Database)}
    Sử dụng \textbf{PostgreSQL}.
    Lưu trữ dữ liệu tập trung: \texttt{users}, \texttt{branches}, \texttt{tables}, \texttt{orders},
    \texttt{order\_tables}, \texttt{order\_items}, \texttt{menu\_items}, \texttt{payments}, \texttt{reviews},
    \texttt{employees}.
\end{itemize}

\subsection{Kiến trúc triển khai}

Kiến trúc triển khai tổng thể gồm các thành phần chính:

\begin{itemize}
    \item \textbf{Client (Browser)} -- giao diện theo từng vai trò người dùng.
    \item \textbf{API Server} -- tiếp nhận yêu cầu, kiểm tra xác thực/phân quyền và điều phối xử lý nghiệp vụ.
    \item \textbf{Lớp nghiệp vụ (Service)} -- hiện thực các quy tắc đặt bàn, vận hành phục vụ, thanh toán và quản trị.
    \item \textbf{Lớp truy cập dữ liệu (ORM)} -- ánh xạ thực thể và thực thi truy vấn trên CSDL.
    \item \textbf{Cơ sở dữ liệu quan hệ} -- lưu trữ dữ liệu và ràng buộc toàn vẹn.
    \item \textbf{Kênh realtime} -- đồng bộ trạng thái món ăn giữa bếp và phục vụ theo chi nhánh (UC08).
\end{itemize}
...
\begin{itemize}
    \item \textbf{Separation of Concerns}: mỗi tầng đảm nhiệm một vai trò độc lập (View / Controller / Service / Model).
    \item \textbf{Loose Coupling}: frontend chỉ phụ thuộc API contract, không truy cập trực tiếp CSDL.
    \item \textbf{Role-based Access}: phân quyền theo \texttt{users.role} và phạm vi \texttt{branch\_id}.
    \item \textbf{Real-time synchronization}: trạng thái món ăn được cập nhật liên tục qua WebSocket theo chi nhánh.
    \item \textbf{Interval-based booking}: giữ chỗ bàn dựa trên khoảng thời gian có hạn (buffer 2 giờ), không chỉ mốc \textit{arrival\_time} đơn lẻ.
\end{itemize}
...
theo thời gian cho ba quy trình nghiệp vụ trọng tâm. Các biểu đồ bám sát luồng UC05--UC07 (đặt bàn, gọi món)
và UC11 (thanh toán), thể hiện rõ các bước xử lý chính và cập nhật dữ liệu.

\begin{figure}[H]
    \centering
    \includegraphics[width=\textwidth]{Chuong/3.1-quy-trinh-dat-ban.png}
...
\begin{figure}[H]
    \centering
    \includegraphics[width=\textwidth]{Chuong/3.2-quy-trinh-goi-mon.png}
...
\begin{figure}[H]
    \centering
    \includegraphics[width=\textwidth]{Chuong/3.3-quy-trinh-thanh-toan.png}
...
Biểu đồ lớp mô tả cấu trúc các thực thể nghiệp vụ chính và mối quan hệ giữa chúng
(User, Branch, Table, MenuItem, Order, OrderItem, Payment, Review,...).
Biểu đồ hỗ trợ thống nhất thiết kế CSDL với các lớp xử lý ở tầng nghiệp vụ và dữ liệu.
...
Hình~\ref{fig:erd} mô tả sơ đồ cơ sở dữ liệu tổng thể gồm \textbf{10 bảng}: \textbf{7 bảng lõi}
(users, branches, tables, orders, order\_tables, order\_items, menu\_items) và \textbf{3 bảng phụ trợ}
(payments, reviews, employees) phục vụ thanh toán, đánh giá và nhân sự.

\textbf{Nguyên tắc thiết kế:} mỗi \texttt{orders} là một \textbf{phiên phục vụ} (đặt bàn trước, walk-in hoặc tại bàn):
...
\begin{figure}[H]
    \centering
    \includegraphics[width=\textwidth]{Chuong/BDquanhe.png}
    \caption{Lược đồ quan hệ của cơ sở dữ liệu}
    \label{fig:erd}
\end{figure}

\subsection{Thuật toán kiểm tra xung đột giữ chỗ (buffer 2 giờ)}

Khi khách đặt bàn hoặc nhân viên tạo walk-in, hệ thống gán mỗi phiên một khoảng chiếm bàn:
\[
    \mathit{hold\_start} = \mathit{arrival\_time}, \qquad
    \mathit{hold\_end} = \mathit{expected\_end\_time} = \mathit{arrival\_time} + 2\,\text{giờ}.
\]
Một bàn \textbf{khả dụng} cho yêu cầu mới tại thời điểm $T$ nếu không tồn tại phiên active
(\texttt{reservation}, \texttt{walk\_in} hoặc \texttt{dine\_in}; trạng thái chưa \texttt{cancelled}/\texttt{completed}/\texttt{no\_show})
mà khoảng $[\mathit{hold\_start},\; \mathit{hold\_end}]$ giao với $[T,\; T+2\,\text{giờ}]$.

\textbf{Ví dụ:} cao điểm 20h toàn bộ bàn đều bận đến 22h thì khách đặt 21h \textbf{không} được gán lại các bàn đó;
chỉ đặt 21h thành công khi còn bàn có khoảng rảnh chứa mốc 21h (ví dụ phiên walk-in 18h kết thúc giữ bàn lúc 20h).
Việc kiểm tra và gán bàn được thực hiện trong giao dịch (transaction) để đảm bảo nhất quán dữ liệu;
với nhóm đông, hệ thống có thể ghép nhiều bàn liền kề đủ sức chứa và ghi liên kết vào \texttt{order\_tables}.

\subsection{Thiết kế các bảng dữ liệu chính}
...
\begin{itemize}
    \item Một chi nhánh có nhiều bàn, món ăn, nhân viên và phiên phục vụ (\texttt{orders}).
    \item Một \texttt{orders} gồm nhiều \texttt{order\_items}; có thể liên kết nhiều \texttt{tables} qua \texttt{order\_tables} khi ghép bàn.
    \item Một \texttt{orders} có tối đa một \texttt{payments} và một \texttt{reviews}.
    \item \texttt{orders.expected\_end\_time} phục vụ kiểm tra xung đột giữ chỗ; \texttt{checked\_in\_at} phân biệt no-show với phiên đã phục vụ.
    \item \texttt{orders.total\_amount} được tính từ \texttt{order\_items.price $\times$ quantity};
    \texttt{payment\_status} trên \texttt{orders} phản ánh trạng thái tổng quát, chi tiết giao dịch ở \texttt{payments.status}.
    \item Các quan hệ được ràng buộc bằng khóa ngoại nhằm đảm bảo tính toàn vẹn tham chiếu.
\end{itemize}
...
trong môi trường nhà hàng. Trong phạm vi báo cáo, nhóm trình bày các màn hình tiêu biểu đại diện cho
các nghiệp vụ chính thay vì liệt kê toàn bộ màn hình chi tiết.
...
\subsubsection{Giao diện trang menu dành cho khách hàng}

Màn hình hỗ trợ khách xem danh sách món theo chi nhánh (UC04): tên món, mô tả, giá, ảnh và trạng thái còn bán.
Khách có thể lọc theo danh mục và chọn món trước khi đặt bàn hoặc gọi món tại bàn.
...
Màn hình đặt bàn (UC05) cho phép chọn chi nhánh, ngày/giờ, số khách và ghi chú.
Hệ thống kiểm tra bàn trống theo buffer 2 giờ, hiển thị gợi ý ``còn chỗ'' hoặc ``hết chỗ'' trước khi xác nhận;
đồng thời áp dụng CAPTCHA để giảm nguy cơ spam tự động.

\begin{figure}[H]
    \centering
    \includegraphics[width=\textwidth]{Chuong/TableBookingUI.png}
    \caption{Giao diện đặt bàn}
    \label{fig:ui-booking}
\end{figure}

\subsubsection{Giao diện quản lý bàn cho nhân viên phục vụ}

Dành cho nhân viên phục vụ (UC06, UC10): xem sơ đồ/trạng thái bàn, tiếp nhận khách walk-in,
xác nhận phiên đặt trước, xử lý no-show và chuyển bàn sang trạng thái phục vụ.
...
\subsubsection{Giao diện bếp xử lý món ăn}

Màn hình bếp (UC08) tối giản, ưu tiên danh sách món cần chế biến theo thứ tự thời gian.
Nhân viên bếp cập nhật trạng thái \texttt{processing} $\rightarrow$ \texttt{done}; phục vụ nhận cập nhật realtime.

\begin{figure}[H]
    \centering
    \includegraphics[width=\textwidth]{Chuong/UIDATN/kitchenUI.png}
    \caption{Giao diện bếp xử lý món ăn}
    \label{fig:ui-kitchen}
\end{figure}

\subsubsection{Giao diện quản lý chi nhánh}

Tập trung giám sát vận hành (UC14, UC15): thống kê nhanh doanh thu, số phiên phục vụ,
menu sidebar tới quản lý thực đơn, báo cáo và đánh giá khách hàng.

\begin{figure}[H]
    \centering
    \includegraphics[width=\textwidth]{Chuong/adminUI.png}
    \caption{Giao diện quản lý dành cho quản lý chi nhánh}
    \label{fig:ui-admin}
\end{figure}

\subsection{Đánh giá thiết kế giao diện}

Nhìn chung, giao diện được xây dựng theo hướng phân vai trò rõ ràng, đảm bảo mỗi nhóm người dùng
chỉ thao tác trên chức năng phù hợp nghiệp vụ. Việc chọn các màn hình tiêu biểu giúp nội dung báo cáo
ngắn gọn nhưng vẫn phản ánh định hướng thiết kế. Các giao diện đáp ứng yêu cầu về tính trực quan,
thống nhất và thao tác nhanh trong môi trường nhà hàng; đồng thời tạo nền tảng thuận lợi cho việc
bổ sung màn hình và tính năng ở các giai đoạn tiếp theo.
